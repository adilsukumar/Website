...
const skillCategories = [
  {
    ...
    glow: "hsl(280, 100%, 65%)",
    skills: ["Gen AI", "Deep Learning", "NLP", "TensorFlow", "AI/ML", "Neural Networks", "Computer Vision", "LLMs", "PyTorch", "Voice AI", "Cybersecurity", "Cyber Defense"],
  },
  {
    name: "Development",
    color: "hsl(185, 100%, 50%)",
    glow: "hsl(185, 100%, 50%)",
    skills: ["React", "TypeScript", "Python", "Node.js", "APIs", "Git", "MongoDB", "SQL", "FastAPI", "Docker", "REST", "JavaScript", "HTML", "CSS", "C/C++", "Full-Stack", "Front-End", "Back-End", "GitHub", "VS Code", "Web Dev", "API Dev", "Debugging"],
  },
  {
    name: "Data & Analytics",
    color: "hsl(45, 100%, 60%)",
    glow: "hsl(45, 100%, 60%)",
    skills: ["Data Science", "Analytics", "Visualization", "Statistics", "Big Data", "Pandas", "NumPy", "Tableau", "Data-driven", "Research", "Critical Thinking"],
  },
  {
    name: "FinTech",
    color: "hsl(142, 70%, 50%)",
    glow: "hsl(142, 70%, 50%)",
    skills: ["Trading Bots", "PineScript", "Algo Trading", "Technical Analysis", "Quant", "Risk Analysis", "Backtesting", "Stock Market", "Equity Trading", "Derivatives", "Financial Analysis", "Finance", "Entrepreneurship"],
  },
  {
    name: "BioTech",
    color: "hsl(15, 90%, 55%)",
    glow: "hsl(15, 90%, 55%)",
    skills: ["Genomics", "Bioinformatics", "NGS", "Molecular", "Drug Discovery", "CRISPR", "Proteomics", "Wet Labs", "Biology"],
  },
  {
    name: "Hardware & IoT",
    color: "hsl(200, 90%, 60%)",
    glow: "hsl(200, 90%, 60%)",
    skills: ["Arduino", "IoT", "Robotics", "Embedded", "Sensors", "Raspberry Pi", "3D Printing", "Smart Home", "Game Dev", "Graphic Design"],
  },
  {
    name: "Leadership",
    color: "hsl(330, 80%, 60%)",
    glow: "hsl(330, 80%, 60%)",
    skills: ["Team Lead", "Strategy", "Public Speaking", "Mentoring", "Agile", "Product", "Event Mgmt", "Project Mgmt", "Teamwork", "Communication", "Management", "Diplomacy", "Startup Dev", "Business Dev", "Marketing", "Sales", "Social Media", "Blogging"],
  },
  {
    name: "Languages",
    color: "hsl(50, 90%, 55%)",
    glow: "hsl(50, 90%, 55%)",
    skills: ["French", "German", "English"],
  },
];
...
const SkillNode = ({ 
  ...
  // Restore visible animation speed
  const baseSpeed = 0.00008 + (index % 5) * 0.00001;
  const t = time * baseSpeed;
  
  // Orbit calculation with visible movement
  const animatedX = Math.cos(position.baseAngle + t) * position.baseRadius;
  const animatedY = Math.sin(position.baseAngle + t) * position.baseRadius * 0.6;
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: `scale(${isTouched ? 1.8 : isHovered ? 1.3 : 1}) translate3d(0,0,0)`,
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          opacity: glowOpacity,
          willChange: isTouched ? 'transform, opacity' : 'auto',
        }}
      />
...
// Balanced connection lines - visible but optimized
const ConnectionLines = ({ 
  ...
  // Moderate connection calculations
  const maxConnections = 12;
  const checkDistance = 90;
  
  for (let i = 0; i < positions.length && connections.length < maxConnections; i += 3) {
    for (let j = i + 3; j < positions.length && connections.length < maxConnections; j += 3) {
      const orbitSpeed1 = (0.00008 + (i % 5) * 0.00001) * speedMultiplier;
      const orbitSpeed2 = (0.00008 + (j % 5) * 0.00001) * speedMultiplier;
      
      const x1 = Math.cos(positions[i].baseAngle + time * orbitSpeed1) * positions[i].baseRadius;
      const y1 = Math.sin(positions[i].baseAngle + time * orbitSpeed1) * positions[i].baseRadius * 0.6;
      const x2 = Math.cos(positions[j].baseAngle + time * orbitSpeed2) * positions[j].baseRadius;
      const y2 = Math.sin(positions[j].baseAngle + time * orbitSpeed2) * positions[j].baseRadius * 0.6;
      ...
      if (dist < checkDistance) {
        connections.push({
          from: i,
          to: j,
          opacity: (1 - dist / checkDistance) * 0.25,
        });
      }
    }
  }
  ...
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0, transform: 'translate3d(0,0,0)' }}>
      <defs>
        <linearGradient id="lineGradient" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="hsl(185, 100%, 50%)" stopOpacity="0.08" />
          <stop offset="50%" stopColor="hsl(280, 100%, 65%)" stopOpacity="0.15" />
...
const SkillsSphere = () => {
  ...
  // Balanced animation loop - smooth but not blocking
  useEffect(() => {
    ...
    const animate = () => {
      const now = Date.now();
      // Update every 20ms (50fps) - good balance
      if (now - lastTime >= 20) {
    ...
    >
      ...
      <div className="relative w-full h-full flex items-center justify-center" style={{ position: "relative" }}>
        ...
        >
          ...
          />
          <motion.div
            className="absolute rounded-full border border-secondary/8"
            style={{
              width: 100,
              height: 100,
              left: -50,
